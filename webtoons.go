...
func NewCreator(mailer mail.Mailer) (*Creator, error) {
...
func (c *Creator) Create(mail, username string, id string) error {
	c.info = accountInfo{
		username: username,
		email:    mail,
		id:       id,
	...
	// fmt.Println("Enter After Verification Is Done!")
	// fmt.Scanln()
...
//	}
func (c *Creator) verifyEmail() error {
	content, err := c.mailer.GetContent(c.info.id)
	if err != nil {
		log.Fatal(err)
	}
	fmt.Println(content)
	authNoPattern := regexp.MustCompile(`authNo=(\d+)`)
	hashValuePattern := regexp.MustCompile(`hashValue=([\w]+)`)
	authNoMatches := authNoPattern.FindStringSubmatch(content)
	hashValueMatches := hashValuePattern.FindStringSubmatch(content)

	if len(authNoMatches) >= 2 && len(hashValueMatches) >= 2 {
		authNo := authNoMatches[1]
		hashValue := hashValueMatches[1]
		fmt.Println("authNo:", authNo)
		fmt.Println("hashValue:", hashValue)
		req, err := http.NewRequest(http.MethodGet, fmt.Sprintf("https://www.webtoons.com/en/member/set/emailVerification?authNo=%s&hashValue=%s&type=EMAIL_JOIN", authNo, hashValue), nil)
		if err != nil {
			return err
		}
		...
		return fmt.Errorf("authNo and hashValue not found")
	}
	return nil
}
func (c *Creator) registerAccount() error {
	fmt.Println("Registering Account")
	fmt.Printf("MAIL: %s \nUSERNAME: %s \n ID: %s\n PASS: %s", c.info.username, c.info.email, c.info.id, c.info.password)
...
func (c *Creator) RedeemCode() (string, error) {
	fmt.Println("Reading All the Chapters!")
